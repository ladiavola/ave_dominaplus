...
async def test_call_bridge_returns_data_on_200(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.ave_dominaplus.web_server.aiohttp.ClientSession", return_value=session):
...
async def test_tryget_mac_address_parses_valid_xml(hass: HomeAssistant) -> None:
    ...
    server = _new_server(hass)
    session = _FakeSession(_FakeResponse(200, "<root><macaddress>AA:BB:CC:DD:EE:FF</macaddress></root>"))
...
async def test_tryget_mac_address_returns_none_on_missing_tag(hass: HomeAssistant) -> None:
...
async def test_tryget_mac_address_returns_none_on_invalid_xml(hass: HomeAssistant) -> None:
...
async def test_tryget_systeminfo_returns_empty_on_invalid_xml(hass: HomeAssistant) -> None:
